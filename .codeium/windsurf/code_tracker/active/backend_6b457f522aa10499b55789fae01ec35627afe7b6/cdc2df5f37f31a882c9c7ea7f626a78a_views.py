...
class UserViewSet(ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == 'list':
            return UserSerializer  # ä½¿ç”¨ UserSerializer ä»¥åŒ…å«æƒé™å’ŒçŠ¶æ€ä¿¡æ¯
        elif self.action == 'update':
            return UserSerializer
        return UserDetailSerializer

    def update(self, request, *args, **kwargs):
        """æ›´æ–°ç”¨æˆ·ä¿¡æ¯ï¼Œä»…ç®¡ç†å‘˜å¯ç”¨"""
        if self.permission not in request.user.permissions:
            return Response(
                {'error': 'åªæœ‰ç®¡ç†å‘˜å¯ä»¥ä¿®æ”¹ç”¨æˆ·ä¿¡æ¯'},
    ...
    def toggle_active(self, request, pk=None):
        """åˆ‡æ¢ç”¨æˆ·å°ç¦çŠ¶æ€ï¼Œä»…ç®¡ç†å‘˜å¯ç”¨"""
        if self.permission not in request.user.permissions:
            return Response(
                {'error': 'åªæœ‰ç®¡ç†å‘˜å¯ä»¥å°ç¦/è§£å°ç”¨æˆ·'},
                status=HTTP_401_UNAUTHORIZED
            )
...
¿Á ÁÉ*cascade08
É™ ™§*cascade08
§°V "(6b457f522aa10499b55789fae01ec35627afe7b62%file:///root/backend/oj_user/views.py:file:///root/backend