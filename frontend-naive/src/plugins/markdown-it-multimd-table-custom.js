...
module.exports = function multimd_table_plugin(md, options) {
  ...
  function table(state, startLine, endLine, silent) {
    ...
    console.log('[MultimdTable] Processing table tokens...'); // Debug log
    ...
    for (r = 0; r < tableToken.meta.tr.length; r++) {
      ...
      for (c = 0; c < trToken.meta.bounds.length - 1; c++) {
        ...
        console.log(`[MultimdTable] Cell [${r},${c}] text: '${text}' trim: '${text.trim()}' rowspan: ${options.rowspan} hasUpToken: ${!!upTokens[c]}`);

        if (options.rowspan && upTokens[c] && text.trim() === '^') {
          console.log('[MultimdTable] Rowspan MERGE triggered!');
